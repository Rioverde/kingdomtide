...
// Bounds exposes regionBounds for the catalog-coverage tests in other
// packages. The returned value is a snapshot; callers must not mutate the
// underlying maps.
func Bounds() naming.Bounds {
	return regionBounds
}

// Name produces a deterministic structured name for a region. Same
// (character, biome, seed, sc) inputs always return the same Parts — the
// naming package owns body-seed derivation, so the client can reproduce
// any language's body locally from Name.BodySeed.
//
...
// uint64 suitable for seeding rand.NewPCG's second input. Thin wrapper over
// genprim.HashCoord preserved for the distribution-regression test; the
// production naming path uses the identical-shaped mixer inside
// internal/game/naming with distinct primes.
func hashCoord(sc geom.SuperChunkCoord) uint64 {
	return genprim.HashCoord(sc)
}
