...
// provably distinct from every salt used by anchor and region streams.
const seedSaltLandmarkRaw uint64 = 0x3c6ef372fe94f82b

// seedSaltLandmark decorrelates the landmark RNG stream from anchor and
// region streams. Routed through genprim.ToInt64 so future re-tunings
// with a top-bit-set value keep the declaration shape identical and
// avoid a constant-overflow trap.
var seedSaltLandmark = genprim.ToInt64(seedSaltLandmarkRaw)
...
// from the genprim hash primes so the mixer cannot collapse when sc.X
// and subID coincide.
const landmarkSubCellPrime uint64 = 0xd1b54a32d192ed03

// TerrainSampler is the minimal consumer-side interface
// NoiseLandmarkSource needs from a world generator: a terrain lookup for
// water / biome-family tests, and an elevation lookup for kind affinity
// (Tower requires mountain, Chasm requires a local slope). Declared here
// so the landmark package does not depend on the concrete
// *worldgen.WorldGenerator — production wiring passes the generator,
// which satisfies the interface structurally.
type TerrainSampler interface {
	TileAt(x, y int) world.Tile
	ElevationAt(x, y int) float64
}
...
// after construction and the TerrainSampler + RegionSource contracts
// both already require concurrent-read safety.
...
// NewNoiseLandmarkSource wires a landmark source to a TerrainSampler
// (for terrain / elevation sampling) and a RegionSource (for region-
...
func (s *NoiseLandmarkSource) landmarkForSubCell(
	...
	for _, cand := range candidates {
		tile := s.terrain.TileAt(cand.X, cand.Y)
		if isWaterTile(tile) {
			continue
		}
		if !firstDryFound {
		...
		elev := s.terrain.ElevationAt(cand.X, cand.Y)

		kind, ok := s.pickKind(rng, cand, tile, elev, character)
		if !ok {
			continue
		}
		name := Name(kind, character, s.seed, cand)
	...
	name := Name(world.LandmarkShrine, character, s.seed, firstDry)
...
// Exported so tests can assert fitsTerrain against the same gradient
// the production path computed.
func (s *NoiseLandmarkSource) ElevationGradient(cand geom.Position, here float64) float64 {
	var best float64
	offsets := [...][2]int{{-1, 0}, {1, 0}, {0, -1}, {0, 1}}
	for _, off := range offsets {
		e := s.terrain.ElevationAt(cand.X+off[0], cand.Y+off[1])
		d := here - e
		if d < 0 {
			d = -d
		}
...
// Terrain returns the TerrainSampler wired into this source. Tests use
// it to probe the same terrain pipeline the source consulted at
// placement time.
func (s *NoiseLandmarkSource) Terrain() TerrainSampler {
	return s.terrain
}
...
// genprim.HashCoord's two-prime shape and multiplies the sub-cell id by
// a third distinct prime so adjacent sub-cells inside the same
// super-chunk do not share a stream.
func hashSubCell(sc geom.SuperChunkCoord, subID int) uint64 {
	h := genprim.HashCoord(sc)
...
// consumer-side world.LandmarkSource interface.
