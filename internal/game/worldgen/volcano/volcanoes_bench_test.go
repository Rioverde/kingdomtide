...
func BenchmarkPickVolcanoAnchors(b *testing.B) {
	...
	sr := genprim.SuperRegion{X: 0, Y: 0}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = volcano.PickVolcanoAnchorsForTest(seed, genprim.SuperRegion{X: sr.X + i, Y: sr.Y}, lm, wg)
	}
}
...
func BenchmarkGrowFootprint(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		_, _, _ = volcano.GrowFootprintForTest(anchor, st, seed, wg, nil)
	}
}
...
func BenchmarkTerrainOverrideAt_Warm(b *testing.B) {
	...
	for x := -1; x <= 1; x++ {
		for y := -1; y <= 1; y++ {
			src.EnsureSuperRegionForTest(genprim.SuperRegion{X: x, Y: y})
		}
	}
