...
// landmarks and volcanoes are nil — neither is consulted by the zonal
...
func collectDeposits(src *resource.NoiseDepositSource, minSCX, minSCY, maxSCX, maxSCY int) []world.Deposit {
	rect := geom.Rect{
...
func TestNoiseDepositSource_DepositsNear_Sorted(t *testing.T) {
	...
	prevDist := -1
	for i, d := range near {
		dist := resource.ChebyshevForTest(d.Position, center)
		if dist > 40 {
