...
// Mirror package-internal constants so the external test file reads
// cleanly without exporting them from the landmark package.
const (
...
// tests stay readable.
func newTestSource(t *testing.T, seed int64) *landmark.NoiseLandmarkSource {
	t.Helper()
	wg := worldgen.NewWorldGenerator(seed)
	regions := region.NewNoiseRegionSource(seed, wg)
	return landmark.NewNoiseLandmarkSource(seed, regions, wg)
}
...
func TestLandmarksInViewportCoverage(t *testing.T) {
	...
	// layout — can only assert a probabilistic bound on viewport
	// coverage. The 60% floor documents actual behaviour; widening the
	// plan to 6-8 landmarks per super-chunk should be revisited in a
	// follow-up.
	const (
		vpW           = 41
		vpH           = 21
...
func viewportHasLandmark(src *landmark.NoiseLandmarkSource, oX, oY, eX, eY int) bool {
...
func TestLandmarksInTerrainFit(t *testing.T) {
	...
	for range trials {
		...
		for _, l := range got {
			...
			tile := src.Terrain().TileAt(l.Coord.X, l.Coord.Y)
			elev := src.Terrain().ElevationAt(l.Coord.X, l.Coord.Y)
			grad := src.ElevationGradient(l.Coord, elev)
			if !landmark.FitsTerrainForTest(l.Kind, tile, elev, grad) {
...
func TestLandmarksInRegionBias(t *testing.T) {
	...
	wg := worldgen.NewWorldGenerator(seed)
	ancientSrc := landmark.NewNoiseLandmarkSource(seed, fixedCharacterSource{character: world.RegionAncient}, wg)
	normalSrc := landmark.NewNoiseLandmarkSource(seed, fixedCharacterSource{character: world.RegionNormal}, wg)
