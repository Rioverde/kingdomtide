...
// NoiseVolcanoSource needs from a world generator: a single-tile
// terrain lookup. The volcano pipeline uses it during anchor acceptance
// (biome weight + water rejection) and ashland growth (coast clamp).
// Declared here so the volcano package does not depend on the concrete
...
// TerrainSampler and a LandmarkSource. Placement is per super-region
// (4x4 super-chunks) and cached once generated — a 3x3 neighbourhood of
...
// NewNoiseVolcanoSource wires a volcano source to a TerrainSampler (for
...
func (s *NoiseVolcanoSource) TerrainOverrideAt(t geom.Position) (world.Terrain, bool) {
	home := genprim.SuperRegionOf(geom.WorldToSuperChunk(t.X, t.Y))
	for dy := -1; dy <= 1; dy++ {
		for dx := -1; dx <= 1; dx++ {
			sr := genprim.SuperRegion{X: home.X + dx, Y: home.Y + dy}
			data := s.ensureSuperRegion(sr)
...
func (d *superRegionData) generate(s *NoiseVolcanoSource, sr genprim.SuperRegion) {
	anchors := pickVolcanoAnchors(s.seed, sr, s.landmarks, s.terrain)
	if len(anchors) == 0 {
	...
	for _, anchor := range anchors {
		...
		core, slope, ashland := growFootprint(anchor, state, s.seed, s.terrain, landmarks)
		if len(core) == 0 {
...
func volcanoState(seed int64, p geom.Position) world.VolcanoState {
	h := genprim.SplitMix64(uint64(seed^seedSaltVolcanoState), uint64(int64(p.X)), uint64(int64(p.Y)))
...
// consumer-side world.VolcanoSource interface.
