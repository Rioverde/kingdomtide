...
// fixedTerrain satisfies TerrainSampler by returning a hard-coded plains
// tile for every coord. Region-source influence tests only care about the
// noise fields and floor-rescale pipeline, not the biome choice — a fixed
// terrain keeps the test deterministic without building a full
// WorldGenerator.
type fixedTerrain struct {
	tile world.Tile
}

func (f fixedTerrain) TileAt(x, y int) world.Tile { return f.tile }

// newRegionSource wires a NoiseRegionSource using a no-op terrain sampler
// so tests below do not drag the full worldgen stack.
func newRegionSource(seed int64) *NoiseRegionSource {
	return NewNoiseRegionSource(seed, fixedTerrain{
		tile: world.Tile{Terrain: world.TerrainPlains},
	})
}
...
func TestNoiseRegionSourceDeterminism(t *testing.T) {
	...
	srcA := newRegionSource(seed)
	srcB := newRegionSource(seed)
...
func TestInfluenceAtVsRegionAt(t *testing.T) {
	src := newRegionSource(0xcafe_babe)
...
func TestInterfaceCompliance(t *testing.T) {
	...
	var _ world.RegionSource = newRegionSource(1)
}
