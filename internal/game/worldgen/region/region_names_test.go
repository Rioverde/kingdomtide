...
// TestRegionNameDeterminism anchors the purity contract of Name — same
// inputs always return the same Parts record, no matter how many times it
// is called in any order.
func TestRegionNameDeterminism(t *testing.T) {
	const seed int64 = 42
	sc := geom.SuperChunkCoord{X: 3, Y: -5}

	want := Name(world.RegionBlighted, biome.FamilyForest, seed, sc)
	for i := range 5 {
		got := Name(world.RegionBlighted, biome.FamilyForest, seed, sc)
		if got != want {
			t.Fatalf("iteration %d: Name = %+v, want %+v", i, got, want)
		}
	}

	// Different seed must give a different body seed (statistically).
	other := Name(world.RegionBlighted, biome.FamilyForest, seed+1, sc)
	if other.BodySeed == want.BodySeed {
		t.Logf("same BodySeed %x for different seeds — unlikely but not impossible", want.BodySeed)
	}
}

// TestRegionNameAllCharactersProduceParts covers the catalogue of region
// characters. Each call must resolve to a Parts record carrying the
// character's key without panicking.
func TestRegionNameAllCharactersProduceParts(t *testing.T) {
	...
	for _, c := range characters {
		p := Name(c, biome.FamilyPlain, seed, sc)
		if p.Character != c.Key() {
			t.Errorf("Name(%s).Character = %q, want %q", c, p.Character, c.Key())
		}
	}
}
...
func TestRegionNameBodySeedVariety(t *testing.T) {
	...
	for i := range total {
		...
		p := Name(world.RegionFey, biome.FamilyForest, seed, sc)
...
func TestRegionNameFormatCoverage(t *testing.T) {
	...
	for i := range total {
		...
		name := Name(world.RegionNormal, biome.FamilyMountain, seed, sc)
