...
func newVolcanicDepositTestSource(tb testing.TB, seed int64) (*resource.NoiseDepositSource, *volcano.NoiseVolcanoSource) {
	tb.Helper()
	...
	return resource.NewNoiseDepositSource(seed, wg, lm, vs), vs
}
...
func collectSlopeTiles(vs *volcano.NoiseVolcanoSource, minSCX, minSCY, maxSCX, maxSCY int) map[geom.Position]world.Volcano {
...
// the full set again.
func volcanoByState(vs *volcano.NoiseVolcanoSource, minSCX, minSCY, maxSCX, maxSCY int) map[world.VolcanoState]world.Volcano {
...
func TestObsidianDepositAt_OnlyOnSlope(t *testing.T) {
	...
	found := 0
	for p := range slopes {
		dep, ok := resource.ObsidianDepositAtForTest(seed, p, vs)
		if !ok {
			continue
		}
		found++
		...
		for y := volcanoScanMinSC; y < volcanoScanMaxSC; y++ {
			for _, v := range vs.VolcanoAt(geom.SuperChunkCoord{X: x, Y: y}) {
				for _, p := range v.CoreTiles {
					if _, ok := resource.ObsidianDepositAtForTest(seed, p, vs); ok {
...
func TestObsidianDepositAt_Density(t *testing.T) {
	...
	tol := 0.10
	if math.Abs(frac-resource.ObsidianDensityFractionForTest) > tol {
		t.Errorf("obsidian density = %.3f over %d slope tiles, want %.3f ± %.2f",
			frac, total, resource.ObsidianDensityFractionForTest, tol)
	}
	t.Logf("seed %d slope=%d obsidian_hits=%d frac=%.3f target=%.3f",
		seed, total, hits, frac, resource.ObsidianDensityFractionForTest)
}
...
func TestSulfurDepositAt_OnlyCoreAdjacent(t *testing.T) {
	...
	for x := volcanoScanMinSC; x < volcanoScanMaxSC; x++ {
		for y := volcanoScanMinSC; y < volcanoScanMaxSC; y++ {
			for _, v := range vs.VolcanoAt(geom.SuperChunkCoord{X: x, Y: y}) {
				for _, p := range v.SlopeTiles {
					dep, ok := resource.SulfurDepositAtForTest(seed, p, vs)
					if !ok {
						continue
					}
					found++
					...
					if !resource.SlopeAdjacentToCoreForTest(p, v) {
...
func TestSulfurDepositAt_StateDependentDensity(t *testing.T) {
		...
		for y := volcanoScanMinSC; y < volcanoScanMaxSC; y++ {
			for _, v := range vs.VolcanoAt(geom.SuperChunkCoord{X: x, Y: y}) {
				for _, p := range v.SlopeTiles {
					...
					b.total++
					if _, hit := resource.SulfurDepositAtForTest(seed, p, vs); hit {
						b.hits++
					}
				}
			}
	...
	check(world.VolcanoDormant, resource.SulfurDormantFractionForTest, 0.15)
...
func TestSulfurDepositAt_NilVolcanoSource_ReturnsFalse(t *testing.T) {
	...
	if dep, ok := resource.SulfurDepositAtForTest(seed, p, nil); ok {
		t.Errorf("sulfurDepositAt(nil vs) returned deposit %+v, want not-found", dep)
	}
	if dep, ok := resource.ObsidianDepositAtForTest(seed, p, nil); ok {
		...
		for y := volcanoScanMinSC; y < volcanoScanMaxSC && !found; y++ {
			for _, v := range vs.VolcanoAt(geom.SuperChunkCoord{X: x, Y: y}) {
				for _, p := range v.SlopeTiles {
					...
					if _, ok := resource.SulfurDepositAtForTest(seed, p, vs); ok {
