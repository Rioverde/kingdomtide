...
func newPointTestSource(tb testing.TB, seed int64) *resource.NoiseDepositSource {
	tb.Helper()
...
// assertions.
func collectPointDeposits(src *resource.NoiseDepositSource, minSCX, minSCY, maxSCX, maxSCY int) []world.Deposit {
	all := collectDeposits(src, minSCX, minSCY, maxSCX, maxSCY)
	out := make([]world.Deposit, 0, len(all))
	for _, d := range all {
		if _, ok := resource.PointMinDistanceForTest(d.Kind); ok {
			out = append(out, d)
		}
	}
	return out
...
func TestPointDeposits_BiomeGate(t *testing.T) {
	...
	for _, d := range deposits {
		...
		if !resource.PointBiomeAcceptsForTest(d.Kind, tile.Terrain) {
			t.Errorf(
...
func TestPointDeposits_MinSpacingPerKind(t *testing.T) {
	...
	for _, sr := range []genprim.SuperRegion{{X: 0, Y: 0}, {X: 1, Y: 0}, {X: 0, Y: 1}, {X: 1, Y: 1}, {X: -1, Y: 0}} {
		deposits := resource.PointDepositsInRegionForTest(seed, sr, wg, lm, vs)
		...
		for kind, positions := range byKind {
			minDist, _ := resource.PointMinDistanceForTest(kind)
...
func TestPointDeposits_KindRaritySortOrder(t *testing.T) {
	...
	deposits := resource.PointDepositsInRegionForTest(seed, genprim.SuperRegion{X: 0, Y: 0}, wg, lm, vs)
	if len(deposits) < 2 {
	...
	for seedOffset := int64(0); seedOffset < seedCount; seedOffset++ {
		...
		for sy := 0; sy < srCount; sy++ {
			for sx := 0; sx < srCount; sx++ {
				deposits := resource.PointDepositsInRegionForTest(seed, genprim.SuperRegion{X: sx, Y: sy}, wg, lm, vs)
				stone := 0
				gems := 0
				for _, d := range deposits {
					switch d.Kind {
...
func TestTileBlocked_WaterRejects(t *testing.T) {
	...
	for y := 0; y < 200; y++ {
		for x := 0; x < 200; x++ {
			...
			if !resource.TileBlockedForTest(p, wg, nil, nil) {
...
func TestTileBlocked_NilLandmarkAndVolcano(t *testing.T) {
	...
	if resource.TileBlockedForTest(land, wg, nil, nil) {
		t.Errorf("land tile %+v unexpectedly blocked with nil lm/vs", land)
	}
	if !resource.TileBlockedForTest(water, wg, nil, nil) {
