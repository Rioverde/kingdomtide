...
// Biome returns the world.Terrain for a tile given normalised elevation, temperature and
...
// rock mountains.
func Biome(elevation, temperature, moisture float64) world.Terrain {
	...
	if elevation < elevationMountain {
		...
		if temperature < temperatureCold && moisture > moistureDry {
			return world.TerrainTaiga
		}
		if temperature > temperatureHot && moisture < moistureDry {
			return world.TerrainDesert
		}
		return world.TerrainHills
	}
	if elevation < elevationSnowyPeak {
		if temperature < temperatureCold {
			return world.TerrainSnow
		}
		return world.TerrainMountain
	}
	return world.TerrainSnowyPeak
}
...
func lowlandBiome(temperature, moisture float64) world.Terrain {
	switch {
	...
	case temperature > temperatureHot:
		...
		if moisture < moistureWet {
			return world.TerrainSavanna
		}
		return world.TerrainJungle
	default:
