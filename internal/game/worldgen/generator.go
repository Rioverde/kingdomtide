...
type WorldGenerator struct {
	...
	// hydrology caches the priority-flood fill, D8 flow accumulation, lake marks
	// and river mask per chunk-centered buffer. Populated lazily on first access;
	// the cache itself owns all concurrency via hashicorp/golang-lru/v2 so the
	// generator stays read-only after construction.
...
func NewWorldGenerator(seed int64) *WorldGenerator {
	...
	return &WorldGenerator{
		...
		hydrology:        newHydrologyCache(DefaultHydrologyCacheCapacity),
	}
}
...
//
// The hydrology field for this chunk's buffer is materialised once and queried
// for both the river and lake overlays in the same loop. Sharing the field
// keeps Chunk() to a single Priority-Flood + flow-accumulation pass per chunk.
