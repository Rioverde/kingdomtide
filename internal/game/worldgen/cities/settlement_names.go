// Package cities hosts settlement-domain naming helpers. Phase 5a will
// promote Settlement to a first-class domain struct with placement
// logic and grow the package accordingly; today it exposes the
// catalog-bounds table and a pure-function name generator the locale
// coverage tests wire against.
package cities
...
// logic; for the naming tier the kind exists only so the pipeline can
...
// settlement's kind_pattern templates. Four stock cultures ship today;
// Phase 5a may subdivide or add more.
type Culture string
...
// Bounds returns the catalog-coverage bounds for the settlement domain.
// The returned value is a snapshot; callers must not mutate the
// underlying maps.
func Bounds() naming.Bounds {
	return settlementBounds
}

// Name produces a deterministic structured name for a settlement. The
// returned Parts is composed into a display string by the client via
// the locale catalog under
