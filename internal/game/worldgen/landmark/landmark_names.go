...
// Bounds exposes landmarkBounds for the catalog-coverage tests in other
// packages. The returned value is a snapshot; callers must not mutate
// the underlying maps.
func Bounds() naming.Bounds {
	return landmarkBounds
}

// Name produces a deterministic structured name for a landmark. Same
// (kind, character, seed, coord) inputs always return the same Parts.
// The returned Parts is stored on world.Landmark and composed into a
// display string by the client via the locale catalog under
// "landmark.name.*" and "landmark.prefix.*" keys.
func Name(
