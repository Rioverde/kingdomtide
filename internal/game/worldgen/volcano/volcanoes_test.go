...
func collectVolcanoes(src *volcano.NoiseVolcanoSource, minSCX, minSCY, maxSCX, maxSCY int) []world.Volcano {
...
func TestNoiseVolcanoSource_PoissonDiskMinSpacing(t *testing.T) {
	...
	minSpacing := volcano.MinSpacingTilesForTest
	...
	for i := range all {
		for j := i + 1; j < len(all); j++ {
			...
			srI := volcano.SuperRegionOfForTest(geom.WorldToSuperChunk(all[i].Anchor.X, all[i].Anchor.Y))
			srJ := volcano.SuperRegionOfForTest(geom.WorldToSuperChunk(all[j].Anchor.X, all[j].Anchor.Y))
			if srI != srJ {
				continue
			}
			...
			if dx*dx+dy*dy < minSq {
				t.Errorf("anchors %+v and %+v within super-region %+v closer than %d tiles (d^2=%d)",
					all[i].Anchor, all[j].Anchor, srI, minSpacing, dx*dx+dy*dy)
			}
		}
	}
}
...
func TestNoiseVolcanoSource_CrossSR_Spillover(t *testing.T) {
	...
	for x := -24; x < 24 && !found; x++ {
		for y := -24; y < 24 && !found; y++ {
			sc := geom.SuperChunkCoord{X: x, Y: y}
			home := volcano.SuperRegionOfForTest(sc)
			for _, v := range src.VolcanoAt(sc) {
				...
				for _, p := range all {
					tileSR := volcano.SuperRegionOfForTest(geom.WorldToSuperChunk(p.X, p.Y))
					if tileSR == home {
						continue
					}
