...
type WorldGenerator struct {
	...
	// drainage caches priority-flood fill + flow-accumulation per buffer-origin chunk.
	// Populated lazily on first access (Phase 3 of the terrain-realism plan). Read-only
	// after construction from the generator's perspective — the cache itself owns all
	// concurrency via hashicorp/golang-lru/v2.
...
func NewWorldGenerator(seed int64) *WorldGenerator {
	...
	return &WorldGenerator{
		...
		drainage:         newDrainageCache(DefaultDrainageCacheCapacity),
	}
}
...
// seedJitter01 returns a deterministic float in [0, 1) derived from (seed, salt). Reuses
// the riverSourceHash mixer (x and y parameters repurposed as the salt split across two
// ints) then takes the top 53 bits of the 64-bit mixer output and divides by 2^53 — the
// standard construction for a uniform [0, 1) float from a 64-bit PRNG output.
//
// We split salt into two ints rather than passing it whole so the underlying mixer still
// sees three non-zero inputs (the mixer XORs three multiplies; zeros collapse one slot).
func seedJitter01(seed int64, salt uint64) float64 {
	saltLo := int(salt & 0xffffffff)
	saltHi := int(salt >> 32)
	u := riverSourceHash(saltLo, saltHi, seed)
	// 53-bit mantissa: shift right by 11, divide by 2^53. Matches math/rand's Float64
	// construction and keeps every representable float64 in [0, 1) reachable.
	return float64(u>>11) / (1 << 53)
}

// Chunk fills an entire Chunk worth of tiles in a single pass: for each
// coord it calls TileAt for the base biome, then overlays the drainage layer
// (rivers + lakes). Biome is intentionally not altered here — river-adjacent
// biome blending is a later tuning pass. Settlements (villages, castles) are
// NOT placed at this layer — they are civilization artifacts and live on
// Layer 3, generated by the history/simulation pipeline (Phase 5 of the
// roadmap).
//
// Phase 3 (drainage): the drainage field for this chunk's 5×5 buffer is
// materialised once and then queried for river tiles, lake tiles, and
// (indirectly through RiverTilesInChunk) flow-accumulation. Sharing the
// field between the river tracer and the lake overlay avoids paying for
// two priority-flood passes per chunk.
func (g *WorldGenerator) Chunk(cc ChunkCoord) Chunk {
	chunk := Chunk{Coord: cc}
	minX, _, minY, _ := cc.Bounds()

	field := g.drainageFor(cc)
	riverTiles := g.riverTilesInChunkOnField(cc, field)

	for dy := range ChunkSize {
		for dx := range ChunkSize {
			x, y := minX+dx, minY+dy
			t := g.TileAt(x, y)
			if _, wet := riverTiles[[2]int{x, y}]; wet {
				t.Overlays |= game.OverlayRiver
			}
			if raised, inBuf := field.isLake(x, y); inBuf && raised {
